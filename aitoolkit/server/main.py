...
# Determine server name based on configuration type
server_name = "ai-dev-toolkit"

# Parse enabled tools from environment variables
def initialize_enabled_tools():
    enabled_tools = os.environ.get("AI_DEV_TOOLKIT_ENABLED_TOOLS", "")
    if enabled_tools:
        return enabled_tools.split(",")
    # Default to all tools if none specified
    return ["file_system", "project_starter", "ai_librarian", "think"]

# Get enabled tools
ENABLED_TOOLS = initialize_enabled_tools()
...
# Only register file system tools if enabled
...
# Parse directories from command line args and environment variables
def initialize_allowed_directories():
    ...
    allowed_dirs.append(repo_dir)
    logger.info(f"Added repository directory: {repo_dir}")
...
# Only register AI Librarian tools if enabled
if "ai_librarian" in ENABLED_TOOLS:
...
# Only register Project Starter tools if enabled
if "project_starter" in ENABLED_TOOLS:
...
# Only register Think Tool if enabled
...
# Register prompts based on enabled tools

@mcp.prompt()
def file_operations_help() -> str:
    """
    Provide help with file system operations.
    """
    return """
    I can help you with file system operations. Here are some examples:
    
    1. Reading files: `read_file("path/to/file.txt")`
    2. Writing files: `write_file("path/to/file.txt", "content")`
    3. Listing directories: `list_directory("path/to/directory")`
    4. Searching files: `search_files("path/to/directory", "search term")`
    
    Let me know what you'd like to do with files and I'll help you.
    """
...
@mcp.prompt()
def project_starter_help() -> str:
    """
    Provide help with project creation.
    """
    return """
    I can help you create new projects. Here's how:
    
    1. Create a project plan: `create_project_plan("Project Name", "Description", "web|cli|library|api", ["Feature 1", "Feature 2"])`
    2. Generate structure: `generate_project_structure(structure_text, "output/directory")`
    3. Create starter files: `create_starter_files("project/directory", "Project Name", "web|cli|library|api")`
    
    Let me know what kind of project you want to create!
    """
